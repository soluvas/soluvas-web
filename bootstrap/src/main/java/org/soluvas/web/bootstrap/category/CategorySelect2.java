...
public class CategorySelect2 extends BootstrapSelect2Choice<Category2> {
...
//	private IModel<List<Category2>> sortedCategoriesModel;
	 ...
	 * @param model
	 */
	public CategorySelect2(String id, @Nullable Category2 currentCategory) {
		super(id);
		setModel(new LoadableCategoryModel(currentCategory));
		setProvider(new CategoryChoiceProvider(null));
	}
	/**
	 * @param id
	 * @param model
	 */
	public CategorySelect2(String id, IModel<Category2> model) {
		super(id, model);
		setProvider(new CategoryChoiceProvider(null));
	}
