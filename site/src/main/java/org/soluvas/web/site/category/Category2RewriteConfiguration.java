...
/**
 * Works in conjunction with {@link Category2RequestMapper}.
 * @author ceefour
 */
@RewriteConfiguration
public class Category2RewriteConfiguration extends HttpConfigurationProvider {

	private static final Logger log = LoggerFactory
			.getLogger(Category2RewriteConfiguration.class);
	
	@Inject
	private TenantBeans<? extends MongoCategoryRepository> categoryRepos;
	@Inject
	...
	public Configuration getConfiguration(ServletContext context) {
		...
		for (Map.Entry<String, TenantState> entry : tenantRepo.getStates().entrySet()) {
			if (entry.getValue() == TenantState.ACTIVE) {
				...
				final MongoCategoryRepository categoryRepo = categoryRepos.get(tenantId);
